...
def build_risk_quantale() -> ResiduatedQuantale:
    # Stage 1 — Set (chain: halted < observe_only < paper_trade < micro_lot < full_risk)
    Q = FiniteSet(["halted", "observe_only", "paper_trade", "micro_lot", "full_risk"])

    # Stage 2 / 3 — Total order via reflexive-transitive closure of the chain
    direct = BinaryRelation(Q, [
        ("halted",       "observe_only"),
        ("observe_only", "paper_trade"),
        ("paper_trade",  "micro_lot"),
        ("micro_lot",    "full_risk"),
    ])
    leq = direct.closure()
    ...
    # Stage 6 — Monoid: ⊗ = meet (= min on chain)
    def meet_op(a: str, b: str) -> str:
        return lat.meet(a, b)

    # Stage 7+8 — Quantale with residuals. Unit is the top element.
    Q_obj = ResiduatedQuantale(Q, leq, meet_op, unit="full_risk")
    return Q_obj

# ─────────────────────────────────────────────────────────────────────────────
# DEMONSTRATION — Forex Risk State Quantale
# ─────────────────────────────────────────────────────────────────────────────

def demo() -> None:
    print("=" * 60)
    print("  Quantale — Algorithmic Trading Risk Demo")
    print("=" * 60)
    ...
    print("\n── Order queries ────────────────────────────────────────")
    pairs = [("halted","full_risk"), ("paper_trade","micro_lot"), ("micro_lot","paper_trade")]
    for a, b in pairs:
    ...
    subsets = [
        ["observe_only","paper_trade","micro_lot"],
        ["halted","observe_only"],
        ["micro_lot","full_risk"],
        ["halted", "observe_only", "paper_trade", "micro_lot", "full_risk"]
    ]
    for s in subsets:
    ...
    for k, v in report.items():
        if k != "counterexamples":
            print(f"  {k}: {v}")
    compositions = [("micro_lot","paper_trade"), ("observe_only","micro_lot"), ("full_risk","micro_lot")]
    ...
    queries = [
        ...
        ("paper_trade", "full_risk",    "max delegation: strat at paper_trade, cap=full_risk"),
        ("full_risk",   "observe_only", "max delegation: strat at full_risk, cap=observe_only"),
        ("micro_lot",   "micro_lot",    "max delegation: strat at micro_lot, cap=micro_lot"),
    ]
    ...
    print("  Query 3 (right_residual): max_delegatable(full_risk, paper_trade) ->",
          q.max_delegatable("full_risk", "paper_trade"))
